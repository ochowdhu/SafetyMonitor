...
\[
\tempSub(\phi) = \begin{cases}
\emptyset & \mbox{ if } \phi \equiv p\\
\{\alpha\} \cup \{\beta\} \cup \tempSub(\alpha) \cup \tempSub(\beta) & \mbox{ if } \phi\equiv\alpha\until_\mathbb{I}\beta|\alpha\since_\mathbb{I}\beta\\
\tempSub(\alpha) \cup \tempSub(\beta) & \mbox{ if } \phi\equiv\alpha\vee\beta\\
\tempSub(\alpha) & \mbox{ if }\phi\equiv\neg\alpha
\end{cases}
\]
...
That is, that if \monitor reports a policy violation, the trace really did violate the policy. 
Second, \emph{promptness}  requires that the algorithm provide a decision for the given policy in a timely fashion (\ie, with $t$ such that $t\leq \wdelay(\policy)$).
...
A strong promptness property exists where the algorithm can decide satisfaction as soon as it is possible (e.g., once a single event that is a part of an eventually occurs). 
%Early satisfaction can be decided when the trace is a good or bad prefix of the given property \cite{Kupferman2001}.
The eager algorithm provides this stronger promptness while the hybrid algorithm discussed in Section \ref{sec:implementation} only provides standard promptness.
%
...
\monitor requires trying to reduce every unreduced residue in the history structures at every time step. In the worst case, \monitor must perform $d$ reductions per policy at every step, where $d$ is the policy's delay. Each top level reduce can require up to $m$ subreductions, for the policy size $m$. Thus \monitor requires $\sum_{p \in P} d_p*m_p$ reductions per trace step  where $P$ is the system specification and $p$ is an individual policy.
