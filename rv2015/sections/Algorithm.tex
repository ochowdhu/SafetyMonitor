...
$$\psi ::=  p \, | \,  \neg \psi \, | \,  \psi_1 \vee \psi_2 \, | \,  \psi_1 \mathcal{U}_{[l,h]} \psi_2 \, | \, \psi_1 \mathcal{S}_{[l,h]} \psi_2 \, | \, \nextmtl \psi \, | \, \lastmtl \psi$$

Policy formulas are denoted by $\psi$. Policy formulas can include two bounded temporal operators: the past-time operator \emph{since} ($\mathcal{S}$) and the future-time operator \emph{until} ($\mathcal{U}$). The \emph{next} and \emph{last} step operators ($\nextmtl$ and $\lastmtl$, respectively) can also be used.
...
The bounded temporal operators include a time bound interval $[l,h]$ where $l,h \in \mathbb{T}$ and $0 \leq l \leq h$. 
...
% next
\sigma, \tau, j &\vDash \nextmtl \psi & \text{ if and only if } & \sigma, \tau, j+1 \vDash \psi \\
% last
\sigma, \tau, j &\vDash \lastmtl \psi & \text{ if and only if } & \sigma, \tau, j-1 \vDash \psi \\
%% until
