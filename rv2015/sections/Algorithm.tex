...
For any evaluation position of the trace $\sigma$,
let us assume we have maximum $L$ positions in $\sigma$ for which
\policy has not yet been reduced to a boolean value.
Note that if the value of $\wdelay(\policy)$ is large, then
$L$ can be as large as $|\sigma|$.
Addtionally, for each temporal subformula $\phi_1\until\phi_2$ of \policy,
we build history structures that
keep track of segments of  positions in $\sigma$ for which $\phi_1$ is true. Let  assume
we have a maximum of $S$ such segments that are relevant for \policy evaluation.
